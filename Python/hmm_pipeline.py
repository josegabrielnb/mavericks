...
of mavericks (only for species with 1 type!) that could not be detected with maverick_miner.py"""

import sys, os, subprocess, re

with open("chaetura_species.txt", 'r') as f:
            spp = f.readlines()
f.closed
...
for sp in spp:
    ...
    for core in recovery_seqs.keys():
        ...
        dir_name = 'Recover_' + core

        os.mkdir(dir_name)

        for fasta in recovery_seqs[core]:

            #Species + '' was changed by Casuarius_casuarius

            table_name1 = re.sub(r'.fa','_watson.txt', fasta)
            cmd1 = 'nhmmer --noali --watson --tblout ' + dir_name + '/' + table_name1 + ' ' + 'Casuarius_casuarius_5prime200.hmm ' + core + '/' + fasta
            subprocess.run(cmd1, shell = True)
            table_name2 = re.sub(r'.fa','_crick.txt', fasta)
            cmd2 = 'nhmmer --noali --crick --tblout ' + dir_name + '/' + table_name2 + ' ' + 'Casuarius_casuarius_5prime200.hmm '  + core + '/' +  fasta
